...
static int decompress_bdev_page(struct zram *zram, struct page *page, u32 index)
{
	...
	if (prio == ZRAM_PRIMARY_COMP) {
		zstrm = swap_compress_stream_get(zram->primary_comp);
		src = kmap_local_page(page);
		ret = swap_decompress(zram->primary_comp, zstrm, src, size,
				      zstrm->local_copy);
		if (!ret)
...
static int read_compressed_page(struct zram *zram, struct page *page, u32 index)
{
	...
	if (prio == ZRAM_PRIMARY_COMP) {
		...
		src = zs_obj_read_begin(zram->mem_pool, handle, size,
					zstrm->local_copy);
		dst = kmap_local_page(page);
		ret = swap_decompress(zram->primary_comp, zstrm, src, size, dst);
		kunmap_local(dst);
...
static int zram_write_page(struct zram *zram, struct page *page, u32 index)
{
	...
	zstrm = swap_compress_stream_get(zram->primary_comp);
	mem = kmap_local_page(page);
	ret = swap_compress(zram->primary_comp, zstrm, mem, &comp_len);
	kunmap_local(mem);
...
#ifdef CONFIG_SWAP_COMPRESS
	swap_comp_destroy(zram->primary_comp);
	zram->primary_comp = NULL;
#endif
			      ...
			      const char *buf, size_t len)
{
	...
	for (prio = ZRAM_PRIMARY_COMP; prio < ZRAM_MAX_COMPS; prio++) {
		...
		if (prio == ZRAM_PRIMARY_COMP) {
			...
			sc = swap_comp_create(zram->comp_algs[prio]);
			if (IS_ERR(sc)) {
