...
static void zram_swap_submit_write(struct swap_io_ctx *ctx)
{
	...
	for (i = 0; i < nr; i++) {
		...
		if (ret) {
			/*
			 * Free the pages of this folio that were already written
			 * to zram (indices base+0 .. base+j-1).  The remaining
			 * pages were never written so their slots are still empty.
			 * Folios written in earlier iterations keep their swap
			 * entries and zram data; swap_write_end() re-dirties them
			 * so they stay in memory and their slots are reclaimed
			 * later via slot_free_notify or the next swap-out.
			 */
			for (int k = 0; k < j; k++) {
				slot_lock(zram, base + k);
				slot_free(zram, base + k);
				slot_unlock(zram, base + k);
			}
			failed = true;
			atomic64_inc(&zram->stats.failed_writes);
