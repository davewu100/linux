 ...
 * @resolved_out: Output array for resolved (offset, size) per field (required)
 *
		    ...
		    struct ks_resolved_field *resolved_out)
{
	...
	if (!struct_addr || !struct_name || !schema || !resolved_out)
		return -EINVAL;
	...
	for (i = 0; i < schema->nr_fields; i++) {
		...
		if (!field_addr) {
			resolved_out[i].offset = 0;
			resolved_out[i].size = 0;
			continue;
		}
