 ...
 * previously resolved field offsets. Entries are permanent - BTF
 * field offsets are immutable during kernel runtime, so no TTL is
 * needed. Cache persists until kernel reboot or manual invalidation.
 */
				       ...
				       const char *field_path)
{
	...
	/* Cache hit - BTF offsets are immutable, no need to check TTL */
	ks_stats.hits++;
	rcu_read_unlock();
	return entry;
}
