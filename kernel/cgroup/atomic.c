...
/**
 * css_atomic_refresh_cache - refresh aggregated (subtree sum) with one tree walk
 * @memcg: the memory cgroup (root of subtree)
 *
 * Single traversal: for each node in subtree (self + descendants), add its
 * local state[] and events[] into agg. Cost O(subtree size), not O(depth).
 */
static void css_atomic_refresh_cache(struct mem_cgroup *memcg)
{
