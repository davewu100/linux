 ...
 * 3. Recomputing stats/events by tree traversal
 * 4. Updating the cache with fresh values
 * 5. Resetting the update counter
 *
...
void css_atomic_flush(struct mem_cgroup *memcg, bool force)
{
	 ...
	 * These functions will traverse the tree, aggregate values, and update the cache.
	 */
	css_atomic_recompute_and_cache_stats(memcg, stats);
	css_atomic_recompute_and_cache_events(memcg, events);
}

/* Check if cache is valid (threshold-based only) */
static bool css_atomic_cache_is_valid(struct mem_cgroup *memcg)
{
	struct memcg_atomic_cache *cache;
	...
	/* Simple validity check - only invalidated by threshold trigger */
	return READ_ONCE(cache->valid);
}
...
static void css_atomic_cache_update_stats(struct mem_cgroup *memcg,
					   const u64 *results)
{
	struct memcg_atomic_cache *cache;
...
static void css_atomic_cache_update_events(struct mem_cgroup *memcg,
...
/* Recompute stats from atomic counters and update cache */
static int css_atomic_recompute_and_cache_stats(struct mem_cgroup *memcg,
						 u64 *results)
{
	memset(results, 0, MEMCG_VMSTAT_SIZE * sizeof(u64));
	/*
	 * Walk entire cgroup tree to aggregate stats.
	 * Return value ignored - visit callback always returns 0 for aggregation.
	 */
	css_atomic_walk(memcg, css_atomic_visit_batch, results);
	css_atomic_cache_update_stats(memcg, results);
	return 0;
}
...
int css_atomic_page_state_batch(struct mem_cgroup *memcg, u64 *results)
{
	...
	if (css_atomic_stats_need_flush(memcg)) {
		atomic_set(&memcg->atomic_stats_updates, 0);
		return css_atomic_recompute_and_cache_stats(memcg, results);
	}
...
/* Recompute events from atomic counters and update cache */
static int css_atomic_recompute_and_cache_events(struct mem_cgroup *memcg,
						  unsigned long *results)
{
	memset(results, 0, NR_MEMCG_EVENTS * sizeof(unsigned long));
	/*
	 * Walk entire cgroup tree to aggregate events.
	 * Return value ignored - visit callback always returns 0 for aggregation.
	 */
	css_atomic_walk(memcg, css_atomic_visit_events_batch, results);
	css_atomic_cache_update_events(memcg, results);
	return 0;
}
...
int css_atomic_events_batch(struct mem_cgroup *memcg, unsigned long *results)
{
	...
	if (css_atomic_events_need_flush(memcg)) {
		atomic_set(&memcg->atomic_events_updates, 0);
		return css_atomic_recompute_and_cache_events(memcg, results);
	}
