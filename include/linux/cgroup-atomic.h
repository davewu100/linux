...
/*
 * Per-node atomic counter (for numa_stat updates).
 * Each node keeps atomic counters for per-node per-cgroup stats.
 */
struct memcg_atomic_counter_per_node {
	atomic64_t		state[NR_VM_NODE_STAT_ITEMS];
} ____cacheline_aligned_in_smp;

/* Per-cgroup atomic counter: local values only; rstat-like threshold flush. */
