...
/*
 * Per-node atomic local counters (for numa_stat updates).
 * Each node keeps atomic counters for per-node per-cgroup stats (local only).
 */
struct memcg_atomic_local_per_node {
	atomic64_t		state[NR_VM_NODE_STAT_ITEMS];
} ____cacheline_aligned_in_smp;

/* Per-cgroup atomic local: this cgroup's counts only (NOT subtree aggregate). */
struct memcg_atomic_local {
...
/* Per-cgroup aggregated stats (self + all descendants) for read path. */
struct memcg_atomic_aggregated {
	...
	/* Aggregated values (self + all descendants) */
