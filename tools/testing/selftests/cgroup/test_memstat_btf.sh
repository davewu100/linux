...
for n in "${FIELD_COUNTS[@]}"; do
	...
	if [ -f "$TEST_PATH/memory.stat.ks.bin" ]; then
		...
		echo "  memory.stat.ks.bin (1st read, cold): $(( (BIN_COLD_END - BIN_COLD_START) / 1000 )) μs"
		STAT_KS_BIN_AVG=$("$HELPER_BIN" "$TEST_PATH/memory.stat.ks.bin" "$READS" "$WARMUP_READS")
		echo "  memory.stat.ks.bin (avg ${READS}x, hot): $STAT_KS_BIN_AVG μs per full-file read"
	else
		STAT_KS_BIN_AVG="-"
	fi
...
echo ""
echo "memory.stat.ks vs memory.stat.ks.bin (μs per full-file read)"
printf "%-8s %-6s %-14s %-18s %-7s\n" "fields" "n" "memory.stat.ks" "memory.stat.ks.bin" "speedup"
printf "%-8s %-6s %-14s %-18s %-7s\n" "---" "---" "---" "---" "---"
for i in "${!KS_FIELDS[@]}"; do
	_su="${KS_STAT_US[$i]}"
	_bu="${KS_BIN_US[$i]:--}"
	if [ -n "$_bu" ] && [ "$_bu" != "-" ] && [ "$_bu" -gt 0 ] 2>/dev/null && [ -n "$_su" ] && [ "$_su" != "-" ] && [ "$_su" -gt 0 ] 2>/dev/null; then
		printf "%-8s %-6s %-14s %-18s %-7s\n" "${KS_FIELDS[$i]}" "${KS_STAT_EFF[$i]}" "$_su" "$_bu" "$((_su / _bu))x"
	else
		printf "%-8s %-6s %-14s %-18s %-7s\n" "${KS_FIELDS[$i]}" "${KS_STAT_EFF[$i]}" "${_su}" "${_bu}" "-"
	fi
done
echo ""
