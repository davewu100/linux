...
build_full_schema() {
	...
	# Prepend "flush" so BTF query path flushes before read (same as default path).
	IFS=','; echo "flush,${schema_fields[*]}" > "$schema_file"; IFS=' '
}
...
compare_outputs() {
...

	{
		...
		while read -r line; do
			...
			local ev_name="${rest%%:*}"
			local ev_idx="${rest##*:}"
			# Kernel outputs events by display name (e.g. pgscan_kswapd), not vmstats.events[N]
			local ev_val="${ks_values["vmstats.events[$ev_idx]"]}"
			[ -z "$ev_val" ] && ev_val="${ks_values[$ev_name]}"
...
# Verify memory.stat.ks.bin has same nr_fields as .ks; optionally print first few values (no value comparison,
# since .ks and .ks.bin are read at different times and stats can change).
# Binary layout: header 12 bytes, then per entry 16 bytes (kind, reserved3, index u32, value u64); value at offset 12+i*16+8.
compare_ks_with_ks_bin() {
	local nr="$1"
	local i
	local -a ks_vals
	local bin_val

	mapfile -t ks_vals < <(grep -v "^#" "$TEST_PATH/memory.stat.ks" | awk '{print $2}')
	[ "${#ks_vals[@]}" -eq "$nr" ] || { echo "${RED}FAIL: .ks has ${#ks_vals[@]} lines, expected $nr${NC}"; return 1; }
	for ((i=0; i<nr; i++)); do
		bin_val=$(od -An -t u8 -j $((20 + i*16)) -N 8 "$TEST_PATH/memory.stat.ks.bin" 2>/dev/null | tr -d ' ')
		[ -n "$bin_val" ] || { echo "${RED}FAIL: .ks.bin entry $i: could not read value${NC}"; return 1; }
	done
	echo "${GREEN}PASS: memory.stat.ks and memory.stat.ks.bin both have $nr fields${NC}"
	# Print first few values from each for inspection (values may differ between reads)
	local show=$(( nr < 3 ? nr : 3 ))
	if [ "$show" -ge 1 ]; then
		echo -n "  .ks (first $show):"
		for ((i=0; i<show; i++)); do echo -n " ${ks_vals[$i]}"; done; echo ""
		echo -n "  .ks.bin (first $show):"
		for ((i=0; i<show; i++)); do
			bin_val=$(od -An -t u8 -j $((20 + i*16)) -N 8 "$TEST_PATH/memory.stat.ks.bin" 2>/dev/null | tr -d ' ')
			echo -n " $bin_val"
		done
...
if [ ! -f "$CGROUP_ROOT/memory.stat.ks.bin" ]; then
    echo "FAIL: memory.stat.ks.bin not found"
    exit 1
fi
echo "PASS: CONFIG_KSERIAL is enabled (memory.stat.ks + memory.stat.ks.bin)"
echo ""
...
echo "=========================================="
echo "Query: All fields via generated schema (flush, then vmstats.state[N], vmstats.events[N])"
echo "Schema includes 'flush' so stats are flushed before read (same as default mode)."
echo ""
...
compare_outputs "$TEST_PATH/memory.stat" "$TEST_PATH/memory.stat.ks" "$SCHEMA_META" \
	"memory.stat vs memory.stat.ks (full BTF)" || true
echo ""
...
WARMUP_READS=1
COLD_READS=1
echo "Each full-file read = one read() of the entire file (all lines); open once, then ${READS}× (lseek(0)+read)."
...
STAT_COLD_US=$(( (STAT_COLD_NS + 500) / 1000 ))
echo "memory.stat (1st full-file read, cold): $STAT_COLD_US μs"
STAT_AVG=$("$HELPER_BIN" "$TEST_PATH/memory.stat" "$READS" "$WARMUP_READS")
STAT_AVG_FMT=$(echo "$STAT_AVG" | awk '{printf "%.1f", $1/10}')
echo "memory.stat (avg ${READS}x, hot): $STAT_AVG_FMT μs per full-file read"
...
for n in "${FIELD_COUNTS[@]}"; do
	...
	echo "Setting filter: ${n}-field + flush (effective: stat=${n_stat})"
	echo "$stat_filter" > "$TEST_PATH/memory.stat.ks"
	...
	STAT_KS_AVG_FMT=$(echo "$STAT_KS_AVG" | awk '{printf "%.1f", $1/10}')
	echo "  memory.stat.ks (avg ${READS}x, hot): $STAT_KS_AVG_FMT μs per full-file read"

	# Same for binary interface (same schema)
	if [ -f "$TEST_PATH/memory.stat.ks.bin" ]; then
		BIN_COLD_START=$(date +%s%N)
		"$HELPER_BIN" "$TEST_PATH/memory.stat.ks.bin" "$COLD_READS"
		BIN_COLD_END=$(date +%s%N)
		BIN_COLD_US=$(( (BIN_COLD_END - BIN_COLD_START + 500) / 1000 ))
		echo "  memory.stat.ks.bin (1st read, cold): $BIN_COLD_US μs"
		STAT_KS_BIN_AVG=$("$HELPER_BIN" "$TEST_PATH/memory.stat.ks.bin" "$READS" "$WARMUP_READS")
		STAT_KS_BIN_AVG_FMT=$(echo "$STAT_KS_BIN_AVG" | awk '{printf "%.1f", $1/10}')
		echo "  memory.stat.ks.bin (avg ${READS}x, hot): $STAT_KS_BIN_AVG_FMT μs per full-file read"
	else
		STAT_KS_BIN_AVG="-"
	fi
	...
	KS_BIN_US+=("${STAT_KS_BIN_AVG:--}")
	echo ""
done

echo "------------------------------------------"
echo "Verify: 64-field output (must be exactly 64 lines)"
echo "------------------------------------------"
...
if [ -f "$TEST_PATH/memory.stat.ks.bin" ]; then
	BIN_NR=$(od -An -t u4 -j 8 -N 4 "$TEST_PATH/memory.stat.ks.bin" 2>/dev/null | tr -d ' ')
	echo "  Verify memory.stat.ks.bin (same schema as .ks): $BIN_NR fields (expected 64)"
	[ "$BIN_NR" = "64" ] || { echo "${RED}FAIL: 64-field .ks.bin got $BIN_NR fields${NC}"; exit 1; }
	compare_ks_with_ks_bin 64 || exit 1
fi
...
echo "=========================================="
echo "${BLUE}Summary: μs per full-file read (open-once, ${READS}× lseek+read)${NC}"
echo "=========================================="
echo "  Each full-file read = one read() of the entire file (n lines). Not per-field."
echo ""
echo "memory.stat / memory.stat.ks / memory.stat.ks.bin (μs per full-file read)"
printf "%-8s %-6s %-12s %-14s %-18s\n" "fields" "n" "memory.stat" "memory.stat.ks" "memory.stat.ks.bin"
printf "%-8s %-6s %-12s %-14s %-18s\n" "---" "---" "---" "---" "---"
STAT_AVG_FMT=$(echo "$STAT_AVG" | awk '{printf "%.1f", $1/10}')
_su="${KS_STAT_US[-1]:--}"
[ "$_su" = "-" ] || [ -z "$_su" ] && _su_fmt="-" || _su_fmt=$(echo "$_su" | awk '{printf "%.1f", $1/10}')
printf "%-8s %-6s %-12s %-14s %-18s\n" "full" "$STAT_LINE_COUNT" "$STAT_AVG_FMT" "$_su_fmt" "-"
for i in "${!KS_FIELDS[@]}"; do
	_su="${KS_STAT_US[$i]}"
	_bu="${KS_BIN_US[$i]:--}"
	[ "$_su" = "-" ] || [ -z "$_su" ] && _su_fmt="-" || _su_fmt=$(echo "$_su" | awk '{printf "%.1f", $1/10}')
	[ "$_bu" = "-" ] || [ -z "$_bu" ] && _bu_fmt="-" || _bu_fmt=$(echo "$_bu" | awk '{printf "%.1f", $1/10}')
	printf "%-8s %-6s %-12s %-14s %-18s\n" "${KS_FIELDS[$i]}" "${KS_STAT_EFF[$i]}" "-" "$_su_fmt" "$_bu_fmt"
done
echo ""
