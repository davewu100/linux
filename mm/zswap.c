			   ...
			   struct zswap_pool *pool)
{
	 ...
	 * that is never used for DMA by zswap itself.
	 *
	 * For a software (scomp) backend, hand them to acomp as virtual
	 * addresses: crypto takes the synchronous virt path directly and we
	 * avoid building scatterlists.
	 *
	 * For a hardware/offload (async) backend, the buffers must be reachable
	 * by the accelerator's DMA, so keep the scatterlist form; a virtual
	 * (nondma) request would otherwise fall back to software and bypass the
	 * hardware entirely.
	 *
