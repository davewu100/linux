...
{
	struct memcg_atomic_local *counter = READ_ONCE(memcg->atomic_local);
	struct memcg_atomic_local_per_node *node_counter =
		READ_ONCE(pn->atomic_local_per_node);
	struct mem_cgroup *iter;
...
unsigned long memcg_atomic_lruvec_page_state(struct lruvec *lruvec, int idx)
{
	...
	node_counter = READ_ONCE(pn->atomic_local_per_node);
	if (!node_counter)
		return 0;

	return atomic64_read(&node_counter->state[i]);
}
...
void memcg_atomic_transfer_to_parent(struct mem_cgroup *memcg)
{
	...
	struct memcg_atomic_local *child_counter;
	struct memcg_atomic_local *parent_counter;
	int i;

	if (unlikely(!parent))
		return;

	child_counter = READ_ONCE(memcg->atomic_local);
	parent_counter = READ_ONCE(parent->atomic_local);
	...
	for_each_node_state(i, N_MEMORY) {
		...
		struct memcg_atomic_local_per_node *child_node_counter;
		struct memcg_atomic_local_per_node *parent_node_counter;
		int j;

		if (unlikely(!pn || !ppn))
			continue;

		child_node_counter = READ_ONCE(pn->atomic_local_per_node);
		parent_node_counter = READ_ONCE(ppn->atomic_local_per_node);
