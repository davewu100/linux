...
/**
 * swap_bdev_can_merge - default can_merge rule for block-based swap
 * @folio: candidate folio.
 * @prev_folio: last folio already queued in the ctx.
 * @prev_folio_size: byte size of @prev_folio.
 * @rw: READ or WRITE for the accumulating batch.
 *
 * Returns true iff @folio's underlying device sector is exactly
 * contiguous with @prev_folio's, and (for WRITE) both folios belong to
 * the same blkcg. Exported so a swap_ops provider can reuse this rule
 * for its .can_merge member when its submit_read / submit_write honours
 * the same bio sector contract (e.g. when the provider falls back to
 * swap_bdev_submit_read()).
 */
