		...
		int rw)
{
	...
	if (rw == WRITE && folio_memcg_charged(folio)) {
		rcu_read_lock();
 ...
 * contiguous with @prev_folio's. Exported so a swap_ops provider can
 * reuse this rule for its .can_merge member when its submit_read /
 * submit_write honours the same bio sector contract (e.g. when the
 * provider falls back to swap_bdev_submit_read()).
 */
