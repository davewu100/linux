...
static void memcg_stat_format(struct mem_cgroup *memcg, struct seq_buf *s)
{
	...
	for (i = 0; i < ARRAY_SIZE(memory_stats); i++) {
		...
		if (atomic_available) {
			...
			seq_buf_printf(s, "%s %llu", name_buf, size_atomic);
		...
		if (unlikely(idx == NR_SLAB_UNRECLAIMABLE_B)) {
			...
			if (atomic_available) {
				...
				seq_buf_printf(s, "slab_atomic %llu", slab_total_atomic);
				seq_buf_printf(s, " (rstat=%llu diff=%lld)\n",
	...
	if (atomic_events_available) {
		...
		unsigned long pgscan_rstat = memcg_events(memcg, PGSCAN_KSWAPD) +
			       ...
			       memcg_events(memcg, PGSCAN_PROACTIVE) +
			       memcg_events(memcg, PGSCAN_KHUGEPAGED);
		seq_buf_printf(s, "pgscan_atomic %lu", pgscan_atomic);
	...
	if (atomic_events_available) {
		...
		unsigned long pgsteal_rstat = memcg_events(memcg, PGSTEAL_KSWAPD) +
				...
				memcg_events(memcg, PGSTEAL_PROACTIVE) +
				memcg_events(memcg, PGSTEAL_KHUGEPAGED);
		seq_buf_printf(s, "pgsteal_atomic %lu", pgsteal_atomic);
		seq_buf_printf(s, " (rstat=%lu diff=%ld)\n",
	...
	for (i = 0; i < ARRAY_SIZE(memcg_vm_event_stat); i++) {
		...
		if (atomic_events_available) {
			...
			snprintf(event_name_buf, sizeof(event_name_buf), "%s_atomic",
				 vm_event_name(memcg_vm_event_stat[i]));
			seq_buf_printf(s, "%s %lu", event_name_buf, count_atomic);
			seq_buf_printf(s, " (rstat=%lu diff=%ld)\n", count, diff_long);
#else
			/* Atomic counter only - output clean format */
			seq_buf_printf(s, "%s %lu\n", vm_event_name(memcg_vm_event_stat[i]),
