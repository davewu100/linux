				     ...
				     struct memcg_stat_ks_config *cfg)
{
	...
	for (i = 0; i < nr; i++) {
		switch (cfg->field_kind[i]) {
		case KS_FIELD_STATE:
			val = memcg_page_state_output(memcg, cfg->field_index[i]);
			break;
		case KS_FIELD_EVENT:
			val = memcg_events(memcg, memcg_vm_event_stat[cfg->field_index[i]]);
			break;
...
static bool memory_stat_ks_field_cb(const char *field_buf, void *ctx)
{
	...
	if (resolve_name_to_index(field_buf, &cfg->field_kind[*j],
				  &cfg->field_index[*j],
				  cfg->field_name[*j],
				  KS_DISPLAY_NAME_LEN)) {
		(*j)++;
...
/* Output only schema-requested vmstats.state[N] rows. */
					  ...
					  struct mem_cgroup *memcg)
{
	...
	for (i = 0; i < ARRAY_SIZE(memory_stats); i++) {
		...
		for_each_node_state(nid, N_MEMORY) {
			struct lruvec *lruvec = mem_cgroup_lruvec(memcg, NODE_DATA(nid));
			u64 size = lruvec_page_state_output(lruvec, memory_stats[i].idx);
					 ...
					 char *buf, size_t nbytes, loff_t off)
{
	...
	return memcg_ks_write_common(of, buf, nbytes, memcg,
				     &memcg->numa_stat_ks_config,
				     memory_numa_stat_ks_prepare,
				     parse_numa_stat_ks_schema,
				     memory_numa_stat_ks_has_content);
}
