#!/bin/bash
# Test script for memory.stat.atomic and memory.numa_stat.atomic interfaces
...
echo "2. Reading memory.stat (rstat backend):"
head -10 "$CGROUP_ROOT/memory.stat" 2>/dev/null || echo "  [Not available]"
echo ""

echo "3. Reading memory.stat.atomic (atomic backend):"
head -10 "$CGROUP_ROOT/memory.stat.atomic" 2>/dev/null || echo "  [Not available - CONFIG_MEMCG_ATOMIC_COUNTER not enabled]"
echo ""
...
if [ -f "$CGROUP_ROOT/memory.numa_stat" ]; then
    echo "   memory.numa_stat (rstat):"
    head -5 "$CGROUP_ROOT/memory.numa_stat" 2>/dev/null || echo "     [Error reading]"
    echo ""
fi

if [ -f "$CGROUP_ROOT/memory.numa_stat.atomic" ]; then
    echo "   memory.numa_stat.atomic (atomic):"
    head -5 "$CGROUP_ROOT/memory.numa_stat.atomic" 2>/dev/null || echo "     [Error reading]"
    echo ""
fi
